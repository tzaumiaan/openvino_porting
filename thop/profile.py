...
def profile(model, inputs, custom_ops={}, verbose=True):
    ...
    def add_hooks(m):
        # fix efficientnet Conv2dStaticSamePadding problem
        # it has Identidy() child which has no computation but leads to 
        # nn.Conv2d skipped
    ...
    for m in model.modules():
        ...
        if isinstance(m, nn.Conv2d):
            pass
        elif len(list(m.children())) > 0:  # skip for non-leaf module
            continue
